...
namespace HY.Devices.Algorithm.TCL_HeFei
{
    ...
    public class HFTCLSurfaceInspectionByDL : AbstractAlgorithm, IAlgorithm
    {

        private static readonly object _lockObj = new object();
        private static HFTCLSurfaceInspectionByDL _instance;
        public static HFTCLSurfaceInspectionByDL Instance
        {
            get
            {
                if (_instance == null)
                {
                    lock (_lockObj)
                    {
                        if (_instance == null)
                        {
                            _instance = new HFTCLSurfaceInspectionByDL();
                        }
                    }
                }
                return _instance;
        ...
        public string saveImgPath => SaveDir + "/srcImgs"; // 文件原图路径
        public override AlgorithmTypes AlgorithmType => AlgorithmTypes.DetectTarget;
        public override Dictionary<string, dynamic> InitParamNames { get; } = new Dictionary<string, dynamic> { { "class_HdlPath", "I:\\算法替换\\fenlei.hdl" }, { "class_HdictPath", "I:\\算法替换\\fenlei.hdict" }, { "defaultImg", "" } };

        public override Dictionary<string, dynamic> ActionParamNames { get; } = new Dictionary<string, dynamic> { { "Image", "" }, { "OutImage", null }, { "minGray", 10 }, { "Area", 1200 }, { "HWindow", null }, { "hv_bimageshow", 1 }, { "hv_bregionshow", 1 } };
        ...
        public override bool Init(Dictionary<string, dynamic> initParameters)
        {
            try
            {
                //SaveDir = initParameters["savePath"].ToString() + "/srcImgs";
                if (!Directory.Exists(SaveDir))
                {
                    Directory.CreateDirectory(SaveDir);
                }
                FenleiInt(initParameters["class_HdlPath"].ToString(), initParameters["class_HdictPath"].ToString(), out hv_WindowhandledictFenlei,
                out hv_DLPreprocessParamFenlei, out hv_DLModelHandleFenlei);
                IsInit = true;
                Dictionary<string, dynamic> actionParams = new Dictionary<string, dynamic> { { "Image", Path.Combine(SaveDir, "logo.jpg") }, { "OutImage", null }, { "minGray", "10" }, { "Area", "1200" }, { "HWindow", null }, { "hv_bimageshow", "1" }, { "hv_bregionshow", "1" } };
        ...
        public void FenleiInt(HTuple hv_DllPath, HTuple hv_DictPath, out HTuple hv_windowhandledict,
    out HTuple hv_DLPreprocessParam, out HTuple hv_DLModelHandle)
        ...
        public override Dictionary<string, dynamic> DoAction(Dictionary<string, dynamic> actionParams)
        {
            ...
            HObject ho_Image, ho_ImageScaled, ho_ImagePyramid;
            HObject ho_ImageSelect, ho_ConnectedRegions, ho_ConnectedRegions1;
            HObject ho_SelectedRegions, ho_RegionUnion, ho_RegionClosing;
            HObject ho_Rectangle1, ho_MultiChannelImage;

            // Local control variables 

            HTuple hv_Savepath = new HTuple(), hv_SavePartPath = new HTuple();
            ...
            HTuple hv_Dict = new HTuple(), hv_Mean = new HTuple();
            HTuple hv_Deviation = new HTuple(), hv_SelctObj = new HTuple();
            HTuple hv_Number = new HTuple(), hv_Width = new HTuple();
            HTuple hv_Height = new HTuple(), hv_Area = new HTuple();
            HTuple hv_Row = new HTuple(), hv_Column = new HTuple();
            HTuple hv_QuexiannumOUT = new HTuple(), hv_QuexianTypeOUT = new HTuple();
            HTuple hv_QuexianAreaOUT = new HTuple(), hv_ResultScoreOUT = new HTuple();
            HTuple hv_QuexianRowOUT = new HTuple(), hv_QuexianColumnOUT = new HTuple();
            // Initialize local and output iconic variables 
            HOperatorSet.GenEmptyObj(out ho_Image);
            HOperatorSet.GenEmptyObj(out ho_ImageScaled);
            HOperatorSet.GenEmptyObj(out ho_ImagePyramid);
            HOperatorSet.GenEmptyObj(out ho_ImageSelect);
            ...
            try
            {
                //***********缺陷坐标*************
                //标注存图路径
                string imagePath = actionParams["Image"];
                SaveDir = imagePath.Substring(0, imagePath.LastIndexOf('/') + 1);
                string fileName = imagePath.Substring(imagePath.LastIndexOf('/') + 1, imagePath.LastIndexOf(".") - imagePath.LastIndexOf('/') - 1);
                hv_Savepath.Dispose();
                hv_Savepath = System.IO.Path.Combine(SaveDir, "标注图"); //"E:/0_wl/TCL合肥/TCL合肥外观/标注图保存";
                if (!Directory.Exists(hv_Savepath))
                {
                    Directory.CreateDirectory(hv_Savepath);
                }
                hv_SavePartPath.Dispose();
                hv_SavePartPath = System.IO.Path.Combine(SaveDir, "PartImage");  //"E:/0_wl/TCL合肥/TCL合肥外观/Part";
                if (!Directory.Exists(hv_SavePartPath))
                {
                    Directory.CreateDirectory(hv_SavePartPath);
                }
                hv_FileName.Dispose();
                hv_FileName = fileName + "_Ret";
                //*******************************
                hv_Sigma1.Dispose();
                hv_Sigma1 = 10.0;
                hv_Sigma2.Dispose();
                ...
                if(File.Exists(hv_ImagePath))
                //if (actionParams["OutImage"]==null)
                {
                ...
                else
                {
                    ...
                    ho_Image = Utils.Ho_ImageHelper.Bitmap2HObject(actionParams["OutImage"]);
                    HObject hb_Image;
                    HOperatorSet.GenEmptyObj(out hb_Image);
                    hb_Image.Dispose();
                    HOperatorSet.CopyImage(ho_Image, out hb_Image);

                }

                //***********返回数据***************
                //缺陷总数量 Quexiannum 是计数器
                //缺陷类别 QuexianType
                //缺陷面积 QuexianArea
                //缺陷得分 ResultScore
                //QuexianRow1 缺陷行1坐标
                //QuexianColumn1 缺陷列1坐标
                //QuexianRow2 缺陷行2坐标
                //QuexianColumn2 缺陷列2坐标
                //Num0 划伤数量
                //Num1 贴纸数量
                //Num2 擦伤数量
                //Num3 脏污数量
                //组织返回数据  类型Dict
                hv_Dict.Dispose();
                HOperatorSet.CreateDict(out hv_Dict);
                HOperatorSet.SetDictTuple(hv_Dict, "Quexiannum", 0);
                HOperatorSet.SetDictTuple(hv_Dict, "QuexianType", new HTuple());
                HOperatorSet.SetDictTuple(hv_Dict, "QuexianArea", new HTuple());
                HOperatorSet.SetDictTuple(hv_Dict, "ResultScore", new HTuple());
                HOperatorSet.SetDictTuple(hv_Dict, "QuexianRow1", new HTuple());
                HOperatorSet.SetDictTuple(hv_Dict, "QuexianColumn1", new HTuple());
                HOperatorSet.SetDictTuple(hv_Dict, "QuexianRow2", new HTuple());
                HOperatorSet.SetDictTuple(hv_Dict, "QuexianColumn2", new HTuple());
                HOperatorSet.SetDictTuple(hv_Dict, "Num0", 0);
                HOperatorSet.SetDictTuple(hv_Dict, "Num1", 0);
                HOperatorSet.SetDictTuple(hv_Dict, "Num2", 0);
                HOperatorSet.SetDictTuple(hv_Dict, "Num3", 0);
                //**********************************
                {
                ...
                ProsessImage(ho_Image, out ho_ImageScaled, 120);
                ho_ImagePyramid.Dispose();
                HOperatorSet.GenGaussPyramid(ho_ImageScaled, out ho_ImagePyramid, "constant",
                    0.5);
                hv_Number.Dispose();
                HOperatorSet.CountObj(ho_ImagePyramid, out hv_Number);
                ho_ImageSelect.Dispose();
                HOperatorSet.SelectObj(ho_ImagePyramid, out ho_ImageSelect, hv_SelctObj);
                hv_Width.Dispose(); hv_Height.Dispose();
                HOperatorSet.GetImageSize(ho_ImageSelect, out hv_Width, out hv_Height);
                ho_ConnectedRegions.Dispose(); ho_ConnectedRegions1.Dispose();
                FftCheck(ho_ImageSelect, out ho_ConnectedRegions, out ho_ConnectedRegions1,
                ...
                HOperatorSet.ClosingRectangle1(ho_RegionUnion, out ho_RegionClosing, 100, 100);
                ...
                defect_classification_function(ho_ImageSelect, ho_ConnectedRegions1, out ho_Rectangle1,
                    hv_Area, hv_Width, hv_Height, hv_Row, hv_Column, hv_SavePartPath, hv_DLPreprocessParamFenlei,
                    hv_DLModelHandleFenlei, hv_Dict);
                ho_MultiChannelImage.Dispose();
                save_biaozhu_image(ho_ImageSelect, out ho_MultiChannelImage, hv_Savepath, hv_FileName,
                    hv_Dict, 1);
                // 返回数据
                hv_QuexiannumOUT.Dispose();
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    hv_QuexiannumOUT = hv_Dict.TupleGetDictTuple(
                        "Quexiannum");
                }
                hv_QuexianTypeOUT.Dispose();
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    hv_QuexianTypeOUT = hv_Dict.TupleGetDictTuple(
                        "QuexianType");
                }
                hv_QuexianAreaOUT.Dispose();
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    hv_QuexianAreaOUT = hv_Dict.TupleGetDictTuple(
                        "QuexianArea");
                }
                hv_ResultScoreOUT.Dispose();
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    hv_ResultScoreOUT = hv_Dict.TupleGetDictTuple(
                        "ResultScore");
                }
                hv_QuexianRowOUT.Dispose();
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    hv_QuexianRowOUT = hv_Dict.TupleGetDictTuple(
                        "QuexianRow1");
                }
                hv_QuexianColumnOUT.Dispose();
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    hv_QuexianColumnOUT = hv_Dict.TupleGetDictTuple(
                        "QuexianColumn1");
                }
                hv_QuexianRowOUT.Dispose();
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                ...
                for (int i = 0; i < hv_QuexiannumOUT; i++)
                {
                    quexianResultInfos.Add(new TargetResult { Area = hv_QuexianAreaOUT[i].D, TypeName = hv_QuexianTypeOUT[i].S, Column1 = hv_QuexianColumnOUT[i].I, Column2 = hv_QuexianColumnOUT[i].I, Row1 = hv_QuexianRowOUT[i].I, Row2 = hv_QuexianRowOUT[i].I, Score = hv_ResultScoreOUT[i].D });
            ...
            {
                results.Add("result", HDevExpDefaultException.ToString());
                return results;
                //throw HDevExpDefaultException;
        ...
        #region Procedure
        // Local procedures 
        public void defect_classification_function(HObject ho_ImageScaled, HObject ho_Image,
            out HObject ho_Rectangle1, HTuple hv_Area, HTuple hv_Width, HTuple hv_Height,
            HTuple hv_Row, HTuple hv_Column, HTuple hv_SavePartPath, HTuple hv_DLPreprocessParamFenlei,
            HTuple hv_DLModelHandleFenlei, HTuple hv_Dict)
        {
            ...
            HObject ho_ImagePartdomain = null;

            // Local control variables 

            HTuple hv_Number1 = new HTuple(), hv_Index2 = new HTuple();
                ...
                if ((int)(new HTuple(hv_Number1.TupleGreater(0))) != 0)
                {
                    ...
                    using (HDevDisposeHelper dh = new HDevDisposeHelper())
                    {
                        ...
                        HOperatorSet.GenRectangle2(out ho_Rectangle1, hv_Row, hv_Column, HTuple.TupleGenConst(
                            hv_Number1, 0), HTuple.TupleGenConst(hv_Number1, 200), HTuple.TupleGenConst(
                            hv_Number1, 200));
                    }
                    //HOperatorSet.DispObj(ho_Rectangle1, hv_ExpDefaultWinHandle);
                    HTuple end_val7 = hv_Number1;
                    HTuple step_val7 = 1;
                    for (hv_Index2 = 1; hv_Index2.Continue(end_val7, step_val7); hv_Index2 = hv_Index2.TupleAdd(step_val7))
                    {
                        ...
                        HOperatorSet.ReduceDomain(ho_ImageScaled, ho_ObjectSelected, out ho_ImageReduced1
                            );
                        hv_cropRow1.Dispose(); hv_cropColumn1.Dispose(); hv_cropRow2.Dispose(); hv_cropColumn2.Dispose();
                        HOperatorSet.SmallestRectangle1(ho_ObjectSelected, out hv_cropRow1, out hv_cropColumn1,
                            out hv_cropRow2, out hv_cropColumn2);
                        ho_ImagePartdomain.Dispose();
                        HOperatorSet.CropPart(ho_ImageScaled, out ho_ImagePartdomain, hv_cropRow1,
                            hv_cropColumn1, 400, 400);
                        //将小图分类
                        hv_DLSample.Dispose();
                        ...
                        HOperatorSet.ApplyDlModel(hv_DLModelHandleFenlei, hv_DLSample, new HTuple(),
                            out hv_DLResult);
                        //按类文件夹保存part小图片信息，图片产品的条码名称和类型保存
                        save_part_picture(ho_ImagePartdomain, hv_SavePartPath, hv_DLResult, hv_Index2);
                        //结果为划伤和条码时，用红色表示。12.6号改为用汉字来分配，这样减少训练的时候的顺序问题。
                        //if (DLResult.classification_class_names[0]='划痕')
                        //if (DLResult.classification_confidences[0]>0.1)
                        //Dict.QuexianType := [Dict.QuexianType,DLResult.classification_class_names[0]]
                        //Dict.QuexianArea := [Dict.QuexianArea,Area[Index2-1]]
                        //Dict.ResultScore := [Dict.ResultScore,DLResult.classification_confidences[0]]
                        //Dict.QuexianRow1 := [Dict.QuexianRow1,cropRow1]
                        //Dict.QuexianColumn1 := [Dict.QuexianColumn1,cropColumn1]
                        //Dict.QuexianRow2 := [Dict.QuexianRow2,cropRow2]
                        //Dict.QuexianColumn2 := [Dict.QuexianColumn2,cropColumn2]
                        //Dict.Num0 := Dict.Num0+1
                        //Dict.Quexiannum := Dict.Quexiannum+1
                        //endif
                        //endif
                        //结果为擦伤时，用蓝色表示。 因为擦伤比较不明显，增加判断分数。
                        //if (DLResult.classification_class_names[0]='擦伤')
                        //if (DLResult.classification_confidences[0]>0.1)
                        //Dict.QuexianType := [Dict.QuexianType,DLResult.classification_class_names[0]]
                        //Dict.QuexianArea := [Dict.QuexianArea,Area[Index2-1]]
                        //Dict.ResultScore := [Dict.ResultScore,DLResult.classification_confidences[0]]
                        ...
                        //endif
                        //结果为脏污时，用绿色表示。
                        //if (DLResult.classification_class_names[0]='脏污')
                        //if (DLResult.classification_confidences[0]>0.1)
                        //Dict.QuexianType := [Dict.QuexianType,DLResult.classification_class_names[0]]
                        //Dict.QuexianArea := [Dict.QuexianArea,Area[Index2-1]]
                        //Dict.ResultScore := [Dict.ResultScore,DLResult.classification_confidences[0]]
                            ...
                            0))).TupleEqual("测试"))) != 0)
                        {
                            if ((int)(new HTuple(((((hv_DLResult.TupleGetDictTuple("classification_confidences"))).TupleSelect(
                                0))).TupleGreater(0.6))) != 0)
                            {
                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    HOperatorSet.SetDictTuple(hv_Dict, "QuexianType", ((hv_Dict.TupleGetDictTuple(
                                        "QuexianType"))).TupleConcat(((hv_DLResult.TupleGetDictTuple("classification_class_names"))).TupleSelect(
                                        0)));
                                }
                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    HOperatorSet.SetDictTuple(hv_Dict, "QuexianArea", ((hv_Dict.TupleGetDictTuple(
                                        "QuexianArea"))).TupleConcat(hv_Area.TupleSelect(hv_Index2 - 1)));
                                }
                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    HOperatorSet.SetDictTuple(hv_Dict, "ResultScore", ((hv_Dict.TupleGetDictTuple(
                                        "ResultScore"))).TupleConcat(((hv_DLResult.TupleGetDictTuple("classification_confidences"))).TupleSelect(
                                        0)));
                                }
                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    HOperatorSet.SetDictTuple(hv_Dict, "QuexianRow1", ((hv_Dict.TupleGetDictTuple(
                                        "QuexianRow1"))).TupleConcat(hv_cropRow1));
                                }
                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    HOperatorSet.SetDictTuple(hv_Dict, "QuexianColumn1", ((hv_Dict.TupleGetDictTuple(
                                        "QuexianColumn1"))).TupleConcat(hv_cropColumn1));
                                }
                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    HOperatorSet.SetDictTuple(hv_Dict, "QuexianRow2", ((hv_Dict.TupleGetDictTuple(
                                        "QuexianRow2"))).TupleConcat(hv_cropRow2));
                                }
                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    HOperatorSet.SetDictTuple(hv_Dict, "QuexianColumn2", ((hv_Dict.TupleGetDictTuple(
                                        "QuexianColumn2"))).TupleConcat(hv_cropColumn2));
                                }
                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    HOperatorSet.SetDictTuple(hv_Dict, "Num1", (hv_Dict.TupleGetDictTuple(
                                        "Num1")) + 1);
                                }
                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    HOperatorSet.SetDictTuple(hv_Dict, "Quexiannum", (hv_Dict.TupleGetDictTuple(
                                        "Quexiannum")) + 1);
                                }
                            }
                        }
        ...
        public void FftCheck(HObject ho_Image, out HObject ho_ConnectedRegions, out HObject ho_ConnectedRegions1,
        ...
        public void ProsessImage(HObject ho_Image, out HObject ho_ImageScaled, HTuple hv_StandardMean)
        {
            ...
            try
            {
                hv_Mean.Dispose(); hv_Deviation.Dispose();
                HOperatorSet.Intensity(ho_Image, ho_Image, out hv_Mean, out hv_Deviation);
                if ((int)(new HTuple(hv_Mean.TupleLess(hv_StandardMean))) != 0)
                {
                    ho_ImageScaled.Dispose();
                    HOperatorSet.ScaleImage(ho_Image, out ho_ImageScaled, 2, 30);
                }
                else
                {
                    ho_ImageScaled.Dispose();
                    HOperatorSet.ScaleImage(ho_Image, out ho_ImageScaled, 1, 30);
                }
        ...
        public void save_biaozhu_image(HObject ho_Image, out HObject ho_MultiChannelImage,
            HTuple hv_Savepath, HTuple hv_FileName, HTuple hv_Dict, HTuple hv_SelctObj)
        {
            ...
            HObject ho_ImageR, ho_ImageG, ho_ImageB, ho_Rectangle1 = null;
            HObject ho_Rectangle2 = null, ho_RegionDifference = null;

            // Local control variables 

            HTuple hv_Width = new HTuple(), hv_Height = new HTuple();
            HTuple hv_SavepathImage = new HTuple(), hv_FileExists = new HTuple();
            // Initialize local and output iconic variables 
            HOperatorSet.GenEmptyObj(out ho_MultiChannelImage);
            ...
            try
            {
                ...
                HOperatorSet.GetImageSize(ho_Image, out hv_Width, out hv_Height);
                ho_ImageR.Dispose();
                HOperatorSet.CopyImage(ho_Image, out ho_ImageR);
                ho_ImageG.Dispose();
                HOperatorSet.CopyImage(ho_Image, out ho_ImageG);
                ho_ImageB.Dispose();
                HOperatorSet.CopyImage(ho_Image, out ho_ImageB);
                ho_MultiChannelImage.Dispose();
                HOperatorSet.Compose3(ho_ImageR, ho_ImageG, ho_ImageB, out ho_MultiChannelImage
                    );

                if ((int)(hv_Dict.TupleGetDictTuple("Quexiannum")) != 0)
                {
                    using (HDevDisposeHelper dh = new HDevDisposeHelper())
                    {
                        ho_Rectangle1.Dispose();
                        HOperatorSet.GenRectangle1(out ho_Rectangle1, (hv_Dict.TupleGetDictTuple(
                            "QuexianRow1")) * hv_SelctObj, (hv_Dict.TupleGetDictTuple("QuexianColumn1")) * hv_SelctObj,
                            (hv_Dict.TupleGetDictTuple("QuexianRow2")) * hv_SelctObj, (hv_Dict.TupleGetDictTuple(
                            "QuexianColumn2")) * hv_SelctObj);
                    }
                    using (HDevDisposeHelper dh = new HDevDisposeHelper())
                    {
                        ho_Rectangle2.Dispose();
                        HOperatorSet.GenRectangle1(out ho_Rectangle2, ((hv_Dict.TupleGetDictTuple(
                            "QuexianRow1")) - 10) * hv_SelctObj, ((hv_Dict.TupleGetDictTuple("QuexianColumn1")) - 10) * hv_SelctObj,
                            ((hv_Dict.TupleGetDictTuple("QuexianRow2")) + 10) * hv_SelctObj, ((hv_Dict.TupleGetDictTuple(
                            "QuexianColumn2")) + 10) * hv_SelctObj);
                    }
                    ho_RegionDifference.Dispose();
                    HOperatorSet.Difference(ho_Rectangle2, ho_Rectangle1, out ho_RegionDifference
                        );
                    HOperatorSet.OverpaintRegion(ho_MultiChannelImage, ho_RegionDifference, (
                ...
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    //try
                    //{
                        hv_SavepathImage = (SaveDir + "/") + hv_FileName;
                    //}
                    //catch (Exception)
                    //{
                ...
                {
                    HOperatorSet.WriteImage(ho_MultiChannelImage, "jpeg 10", 0, hv_SavepathImage);
                }
                else
                {
        ...
        public void save_part_picture(HObject ho_ImagePartdomain, HTuple hv_SavePartPath,
            HTuple hv_DLResult, HTuple hv_Index2)
        {
            ...
            HTuple hv_path = new HTuple(), hv_imagename = new HTuple();
            HTuple hv_FileExists = new HTuple();
            // Initialize local and output iconic variables 
            try
            {
                //按类文件夹保存part小图片信息,所有类别都保存了
                ...
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    hv_path = (hv_SavePartPath + "/") + (((hv_DLResult.TupleGetDictTuple(
                        "classification_class_names"))).TupleSelect(0));
                }
                hv_imagename.Dispose();
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    hv_imagename = (((((((hv_path + "/") + hv_Index2) + "-") + hv_Minute1) + "-") + hv_Second1) + "-") + hv_MSecond1;
                }
                hv_FileExists.Dispose();
                HOperatorSet.FileExists(hv_path, out hv_FileExists);
                if ((int)(hv_FileExists) != 0)
                {
                    HOperatorSet.WriteImage(ho_ImagePartdomain, "jpeg 100", 0, hv_imagename);
                }
                else
                {
