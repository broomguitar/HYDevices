...
namespace HY.Devices.Algorithm.TCL_HeFei
{
    ...
    public class DoorCrack : AbstractAlgorithm, IAlgorithm
    {
        ...
        public override Dictionary<string, dynamic> ActionParamNames { get; } = new Dictionary<string, dynamic> { { "Image", "" }, { "Yscale", 1 }, { "Zscale", 1 }, { "y0", 0 }, { "x0", 0 }, { "y1", 0 }, { "x1", 0 }, { "threolddata1", 255 }, { "bshumenfeng", 1 } };
        ...
        public override Dictionary<string, dynamic> DoAction(Dictionary<string, dynamic> actionParams)
        {
            try
            {
                ...
                DoorCrackResult doorAlignResult = DoorCrackAction(ho_Image, actionParams["Yscale"],actionParams["Zscale"], actionParams["y0"], actionParams["x0"],actionParams["y1"], actionParams["x1"], actionParams["threolddata1"],actionParams["bshumenfeng"]);
        ...
        private DoorCrackResult DoorCrackAction(HObject hb_image, double Yscale, double Zscale, int y0, int x0, int y1, int x1, double threolddata1, int bshumenfeng)
        {
            ...
            try
            {
                ...
                //门缝左边中心点   Row3, Column3
                {
                    ...
                    HOperatorSet.Compose3(hb_image, hb_image, hb_image, out ExpTmpOutVar);
                    hb_image.Dispose();
                    hb_image = ExpTmpOutVar;
                    ho_ImageConverted.Dispose();
                    HOperatorSet.ConvertImageType(hb_image, out ho_ImageConverted, "byte");
                }
                ho_ImageFeng.Dispose();
                HOperatorSet.CopyImage(ho_ImageConverted, out ho_ImageFeng);
                ho_ROI_0.Dispose();
