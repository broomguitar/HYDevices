...
namespace HY.Devices.Algorithm.Haier_TeBing
{
    ...
    public class Classification : AbstractAlgorithm, IAlgorithm
    {
        ...
        public override Dictionary<string, dynamic> InitParamNames { get; }= new Dictionary<string, dynamic> { { "class_HdlPath", "I:/logo/tobing/logo.hdl" }, { "class_HdictPath", "I:/logo/tobing/logo.hdict" } };

        public override Dictionary<string, dynamic> ActionParamNames { get; }= new Dictionary<string, dynamic> { { "Image", "" },{ "TypeName", "BCD-635WVPAU1" } };
        // 分类模型
        ...
        HTuple hv_DLDatasetDetect = new HTuple();
        HTuple hv_DLModelHandleDetect = new HTuple(), hv_DLPreprocessParamDetect = new HTuple();
        ...
        {
            HTuple hv_hdictpathMubiao = new HTuple();
            HTuple hv_hdlpathMubiao = new HTuple();
            try
            {
                ...
                HOperatorSet.ReadDict(initParameters["class_HdictPath"], new HTuple(), new HTuple(), out hv_DLPreprocessParam);
                hv_DLModelHandle.Dispose();
                HOperatorSet.ReadDlModel(initParameters["class_HdlPath"], out hv_DLModelHandle);
                ...
                hv_hdictpathMubiao = "D:/HY/HYDetect/detect.hdict";
                hv_hdlpathMubiao.Dispose();
                hv_hdlpathMubiao = "D:/HY/HYDetect/detect.hdl";


                hv_DLDatasetDetect.Dispose();
                HOperatorSet.ReadDict(hv_hdictpathMubiao, new HTuple(), new HTuple(), out hv_DLDatasetDetect);
                hv_DLModelHandleDetect.Dispose();
                HOperatorSet.ReadDlModel(hv_hdlpathMubiao, out hv_DLModelHandleDetect);
                hv_DLPreprocessParamDetect.Dispose();
                create_dl_preprocess_param_from_model(hv_DLModelHandleDetect, "false", "full_domain",
                    new HTuple(), new HTuple(), new HTuple(), out hv_DLPreprocessParamDetect);
                //每次读取一张
                HOperatorSet.SetDlModelParam(hv_DLModelHandleDetect, "batch_size", 1);
        ...
        public override Dictionary<string, dynamic> DoAction(Dictionary<string, dynamic> actionParams)
        {
            try
            {
                ...
                results.Add("result", Detect(hb_Image, actionParams["TypeName"]));
                return results;
            }
            catch (Exception ex)
            {
        ...
        private string Detect(HObject hb_Image,string TypeName)
        {

            HTuple hv_DLSampleBatch = new HTuple(), hv_DLResultBatch = new HTuple();
            HTuple hv_ClassNames = new HTuple(), hv_Confidences = new HTuple();
            HTuple hv_DLSampleInference = new HTuple(), hv_DLResultDetect = new HTuple();
            HTuple hv_confidenceDetect = new HTuple();
            ...
            try
            {
                ...
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "classification_class_names", out hv_ClassNames);
                hv_Confidences.Dispose();
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "classification_confidences", out hv_Confidences);
                if ((int)(new HTuple(((hv_ClassNames.TupleSelect(0))).TupleNotEqual("NG"))) != 0)
                {
                    if ((int)(new HTuple(((hv_Confidences.TupleSelect(0))).TupleGreaterEqual(
                        80))) != 0)
                    {
                        return ("NG");
                    }
                ...
                //再进行目标检测
                string restring = (hv_DLResultBatch.TupleGetDictTuple(
               "classification_class_names")).TupleSelect(0);

                string path = "Type.txt";
                if (File.Exists(path))
                {
                    List<string> strs = File.ReadAllLines(path).Where(a => !string.IsNullOrEmpty(a)).ToList();
                    //string Typenames = InitParamNames["TypeName"];
                    if (strs.Contains(TypeName.Trim()))
                    {
                        if (restring != "NG")
                        {
                            hv_DLSampleInference.Dispose();
                            gen_dl_samples_from_images(ho_Image, out hv_DLSampleInference);
                            preprocess_dl_samples(hv_DLSampleInference, hv_DLPreprocessParamDetect);
                            hv_DLResultDetect.Dispose();
                            HOperatorSet.ApplyDlModel(hv_DLModelHandleDetect, hv_DLSampleInference, new HTuple(),
                                out hv_DLResultDetect);
                            hv_confidenceDetect.Dispose();
                            HOperatorSet.GetDictTuple(hv_DLResultDetect, "bbox_confidence", out hv_confidenceDetect);
                            hv_Length.Dispose();

                            HOperatorSet.TupleLength(hv_confidenceDetect, out hv_Length);

                            if (hv_Length.I > 0 && hv_confidenceDetect.D > 0.999)
                            {
                                return ((hv_DLResultBatch.TupleGetDictTuple(
               "classification_class_names"))).TupleSelect(0);
                            }
                            else
                            {
