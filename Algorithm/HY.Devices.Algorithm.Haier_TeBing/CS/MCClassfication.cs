...
namespace HY.Devices.Algorithm.Haier_TeBing
{
    ...
    public class MCClassification : AbstractAlgorithm, IAlgorithm
    {
        private static readonly object _lockObj = new object();
        private static MCClassification _instance;
        public static MCClassification Instance
        {
            get
            {
                if (_instance == null)
                {
                    lock (_lockObj)
                    {
                        if (_instance == null)
                        {
                            _instance = new MCClassification();
                        }
                    }
                }
                return _instance;
        ...
        public override Dictionary<string, dynamic> InitParamNames { get; } = new Dictionary<string, dynamic> { { "class_HdlPath", "E:\\Projects\\JiaoNan\\Fenlei_12091.hdl" }, { "class_HdictPath", "E:\\Projects\\JiaoNan\\Fenlei_12091.hdict" } };
        ...
        public override bool Init(Dictionary<string, dynamic> initParameters)
        {
            try
            {
                ...
                HOperatorSet.ReadDict(initParameters["class_HdictPath"],
        ...
        public override Dictionary<string, dynamic> DoAction(Dictionary<string, dynamic> actionParams)
        {
            try
            {
                ...
                results.Add("result", Detect(hb_Image));
                return results;
            }
            catch (Exception ex)
            {
        ...
        private List<string> Detect(HObject hb_Image)
        {
            HTuple hv_DLSample = new HTuple(), hv_DLResult = new HTuple();
            HObject ho_Image;
            // 新加变量
            HTuple hv_confidences = new HTuple();
            HTuple hv_results = new HTuple(), hv_I = new HTuple();
            HTuple hv_result = new HTuple(), hv_Substrings = new HTuple();
            //
            ...
            try
            {
                ...
                hv_results.Dispose();
                hv_results = new HTuple();
                for (hv_I = 0; (int)hv_I <= 4; hv_I = (int)hv_I + 1)
                {
                    hv_result.Dispose();
                    using (HDevDisposeHelper dh = new HDevDisposeHelper())
                    {
                        hv_result = ((hv_DLResult.TupleGetDictTuple(
                            "classification_class_names"))).TupleSelect(hv_I);
                    }
                    hv_Substrings.Dispose();
                    HOperatorSet.TupleSplit(hv_result, "_", out hv_Substrings);
                    using (HDevDisposeHelper dh = new HDevDisposeHelper())
                    {
                        {
                            HTuple
                              ExpTmpLocalVar_results = hv_results.TupleConcat(
                                hv_Substrings.TupleSelect(1));
                            hv_results.Dispose();
                            hv_results = ExpTmpLocalVar_results;
                        }
                    }
                    using (HDevDisposeHelper dh = new HDevDisposeHelper())
                    {
                        {
                            HTuple
                              ExpTmpLocalVar_confidences = hv_confidences.TupleConcat(
                                ((hv_DLResult.TupleGetDictTuple("classification_confidences"))).TupleSelect(
                                hv_I));
                            hv_confidences.Dispose();
                ...
                ResultsList.Add(hv_results[0]);
                ResultsList.Add(hv_results[1]);
                ResultsList.Add(hv_results[2]);
                ResultsList.Add(hv_results[3]);
                ResultsList.Add(hv_results[4]);

                return ResultsList;
                //if (ResultsList.Contains(hv_TrueType))
                //{
                //    return (hv_TrueType);
                //}
                //else
